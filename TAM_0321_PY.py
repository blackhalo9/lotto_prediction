...
def fn_tam_concat_predict_m(r_path: str, f_month: str, res_dic: {}) -> {}:
    ...
    """
    res = pd.read_csv(r_path, sep=" ", header=None).iloc[:, :-1].transpose()
...
def fn_tam_merge_y(tar_res_dic: {}, f_month) -> {}:
    ...
    for target in tar_res_dic:
        if target in [
            ...
            data = f"{DATA_DIR}/INTEGR_METAL_NON_METAL_MONTHLY_{f_month}.csv"
...
def fn_make_tam(r_path: str, f_month: str, target: str, res_path: str):
    ## fn_make_tam(f'{RESULTS_DIR}/{daterecord}/{target_y_name}_predict_13m_0.5_24-04.txt', '2024-04', target_y , f'{RESULTS_DIR}/{daterecord}/TAM_{target_y_name}_{quantile}.csv') ## (생성 txt 파일 위치, 예측 시작 월, target short name, TAM생성 위치)
    """
    메인 함수
